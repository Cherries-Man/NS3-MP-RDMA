...
namespace ns3
{
	...
	{
	public:
		// Time when the RDMA Queue Pair was created
		Time startTime;
		// Source and destination IP addresses and ports
		Ipv4Address sip, dip;
		uint16_t sport, dport;
		// Size of data to be sent over the RDMA Queue Pair
		uint64_t m_size;
		// Sequence numbers for tracking sent and unacknowledged packets
		uint64_t snd_nxt, snd_una; // next seq to send, the highest unacked seq
		// Priority Group (PG) and IP Identification (IPID)
		uint16_t m_pg;
		uint16_t m_ipid;
		// Window size, the bound of on-the-fly packets
		uint32_t m_win;
		// Base Round-Trip Time (RTT) of this Queue Pair
		uint64_t m_baseRtt;
		// Maximum rate for transmission
		DataRate m_max_rate;
		// Variable window size indicator
		bool m_var_win;
		// Soonest time of next send
		Time m_nextAvail;
		// Current window of packets
		uint32_t wp;
